...
static void	set_p_2_in_redirect_lst(t_info *info, char **str_front_p, \
	char **str_back_p, char *str_p)
{
	...
	rest_front_len = ft_strlen(info->sentence_lst->redirect_lst->str) \
	...
	*str_back_p = (char *)ft_calloc(rest_back_len + 1, sizeof(char));
	...
	while (i++ < rest_front_len)
		(*str_front_p)[i - 1] = info->sentence_lst->redirect_lst->str[i - 1];
	(*str_front_p)[i] = '\0';
	i = 0;
...
static char	*get_str_p_in_redirect_lst(t_info *info, char *str_p, int flag)
{
	...
	if (flag == 0)
	{
		...
		{
			str_p_tmp = ft_strnstr(info->sentence_lst->redirect_lst->str, \
...
static char	*change_p_in_redirect_lst(t_info *info, char *str_p)
{
	str_p = get_str_p_in_redirect_lst(info, str_p, 1);
...
int	expand_the_expandable_in_redirect_lst(t_info *info)
{
	...
	str_p = NULL;
	str_p = get_str_p_in_redirect_lst(info, str_p, 0);
	while (str_p)
	{
		...
		if (info->sentence_lst->env_var_lst->next)
			str_p = change_p_in_redirect_lst(info, str_p);
		else
		{
